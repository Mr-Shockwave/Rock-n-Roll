...
distance/angle on the final confirmation frame only after mineral ID passes.
"""
...
def _post_frame(
    ...
    *,
    ...
    camera_object_id = upload_preview_image(image_path, session_id, phase)
    payload: dict = {
    ...
    if camera_object_id:
        payload["camera_object_id"] = camera_object_id
    resp = requests.post(
        fn_url("scan-frame"),
...
def _run_confirmation(session_id: str) -> tuple[list[float], str | None, str | None]:
    ...
    result = _post_frame(session_id, "confirm1", final_path)
    ...
    print(
        ...
        f"qualifying={len(qualifying)} focus={focus_rock!r}"
    )
    ...
    if focus_rock:
        ...
        print(f"[worker] confirm2 conf={c2:.2f} (focused rock only)")
    else:
        c2 = 0.0
...
def run_session(session: dict) -> None:
    ...
    while time.time() < deadline:
        ...
        while time.time() < deadline and not stop_triggered:
            ...
            result = _post_frame(session_id, "scan", image_path)
            conf = result.get("best_confidence", 0)
            action = result.get("action", "continue")
            print(f"[worker] scan frame conf={conf:.2f} action={action}")
            if action == "stop":
                stop_triggered = True
                break
        ...
        if outcome.get("needs_geometry"):
            dist = distance_tool(final_path or capture_frame_only())
            ang = angle_tool(final_path or capture_frame_only())
